...
std::vector<std::shared_ptr<IBS>> ExpIDIBS::compute(std::pair<std::vector<std::shared_ptr<Object>>,std::vector<std::shared_ptr<Object>>> sets){
...
void ExpIDIBS::output(std::vector<std::shared_ptr<IBS>> ibses, std::string path){
    ...
    for(int i = 0; i < ibses.size(); i++) 
    {
        ...
        {
            double similarity = ibses[i]->getSimilarity(*ibses[j]);
            std::cout << "Similarity (" << i << ", " << j << ") = " << similarity << std::endl;
            //DebugLogger::log();
        }
    }
}

std::string ExpIDIBS::getExpPath(){
